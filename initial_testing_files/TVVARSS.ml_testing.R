...
# When U = NULL, you must do head(<par*>, -4) , where <par*> is par.lower, par.upper,
# par, and par.fixed

t_vec[10] <- Sys.time()
set.seed(1)
R_GenSA <- GenSA(fn = TVVARSS.ml, par = head(par, -4), lower = head(par.lower, -4),
                   upper = par.upper,
                   X = t(X), U = NULL, par.fixed = head(par.fixed, -4),
                   control=list(smooth = F, maxit = maxit.SANN))
t_vec[11] <- Sys.time()
set.seed(1)
Rcpp_GenSA <- GenSA(fn = cpp_TVVARSS_ml, par = head(par, -4), lower = head(par.lower, -4),
                      upper = par.upper,
...
message(sprintf('Total time taken: %s minutes',
                round(as.numeric(tail(t_vec, 1) - t_vec[1], units = 'mins'), 2)))

save(R_optim, Rcpp_optim, R_GenSA, Rcpp_GenSA, t_vec, file = 'TVVARSS.ml_testing.RData')
